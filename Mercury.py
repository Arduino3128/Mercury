...
except ImportError: #If you dont have the required modules this error exception will help install them for you
    print ('Do you have all of the needed Modules ?')
    time.sleep(1)
    yn = input('Would You Like To Install Them Now? y/n (I highly recommend you manually install the modules!!!): ')
    if yn == 'n':
        pass
    if yn == 'y':
        ...
        os.system('pip3 install pygoogling')
...
def agreement():
 ...
 for line in term: #for lines in term
     ...
     if ' ' in line: #if not load up terms
         ...
         agree = input(Fore.YELLOW + 'Type "yes" To Agree: ')
         if agree == 'yes': #saves agree
                 ...
                 agreement()
def ddos():
    ip = input(Fore.CYAN + 'Enter an ip address: ')
    bytess = 65500
    try:
        while True:
...
def readme():
    clear()
    readmef=open(x+"/README.md",'r')
    license = open(x+'/License', 'r') #opens file
    file_contents = readmef.read() #reads file
    ...
    mainmenu()
def sub_link():
    url = input(Fore.CYAN + 'Enter an url: ')
    ...
    try:
        for x in links:
            ...
            website = urllib.request.urlopen(x[count])
...
def twitter():
    ...
    user = input('      Enter a Twitter handle: ')
    ...
    try:
        ...
        while True:
            if proxys_num < 22:
                ...
                print(str(prox_even.text) + ':'+str(port_even.text) + '\n', file=prox_txt)
                proxys_num += 2
            if proxys_num == 22:
                global proxys_num2
...
def sourcecode():
    try:
        name = input(Fore.CYAN + 'Enter the file name: ') #Save File
        file = name+'.html'
        URL1=input(Fore.CYAN + 'Enter An Url ') #Url
        html1 = open(file, 'a+') #Writes The File
        html = open(file, 'w')
        response = urllib.request.urlopen(URL1) #Opens Url
        page_source = response.read() #Reads URL
        space()
        print(page_source, file=html) #Change to python3 Syntax
...
def siteexists():
    try:
        site = input(Fore.CYAN + 'Enter a website: ')
...
def brute_force(): #Declares Function
    ...
    data_store = input(Fore.CYAN + 'Would You like to store the data in an html file y/n ')
    username = input(Fore.CYAN + '  What is the Username? ') #Username
    USS = input('       What is the CSS Selector for the Username? ') #CSS Selector Username
    PSS = input('           What is the CSS Selector for the Password? ') #CSS Selector Password
    SSS  = input ('             What is the CSS Selector for the Submit Button? ') #CSS Selector Submit
    url4 = input('                  What is the Url? ') #URL Requests
    if data_store == 'y':
    ...
    while True:
        try:
            ...
            for line in f:
                ...
                print((Fore.CYAN +'# ----------------------------#'))
                if data_store == 'y':
                    print('User: '+username+'Password: '+line +'\n', file=log_file)
                count =+ 1 #Counts +1
...
def admin():
    ...
    website = input(Fore.CYAN + 'Enter a site to scan just www: ')
    type_link = input('Is the link https or http: ')
    count4 = 1
    while True:
                try:
...
def hash():
    ...
    try:
        oghash = input(Fore.CYAN + "\t Please Enter a Word/String To Hash: ") #asks for word
...
def pip_installep(): #change to p so it could by loaded in prompt
    try:
        pip = input(Fore.CYAN + '   What Pip would you like to install: ')
        ...
        prompt()
def hex():
    choice = input(Fore.CYAN + 'Would you like to (e)ncode or (d)ecode? ')
    if choice == 'e':
        encode1()
    if choice == 'd':
        decode1()
def encode1():
    Str = input(Fore.CYAN + 'String to encode: ')
...
def decode1():
    try:
        Str = input(Fore.CYAN + 'String to decode: ' )
...
def ipaddress():
    try:
        url2 = input(Fore.CYAN +"Enter a website url ") #User Input
...
def networksweb():
    url3 = input(Fore.CYAN + 'Enter a host name include https: ')
    count3 = 1
    while True: #loop
                try:
                        req = Request(url3)
...
def webbrowserfunc():
    ...
    PROXY = input(Fore.CYAN + 'Enter a proxy: ')
    options = webdriver.ChromeOptions()
    options.add_argument("--disable-popup-blocking")
    options.add_argument("--ignore-certificate-errors")
    options.add_argument("disable-infobars")
...
def mac():
    try:
        os.system('ifconfig') #For Linux Change to ifconfig -a
...
def pip_installer():
    try:
        pip = input(Fore.CYAN + '   What Pip would you like to install: ') #what pip
        ...
        mainmenu()
def github():
    githublink = input(Fore.CYAN + '    Enter an url to the GitHub repo: ')
    name = input('      What is the name of the repo? ')
    try:
...
def emailspoofsetup():
    ...
    to = input(Fore.CYAN + "Enter the recipient address: ")
...
def geoLocation():
    try:
        target = input(Fore.CYAN + '    Enter an ip address: ') #User Input
        send_url = 'http://freegeoip.app/json/'+target #Finds Targets
        ...
        mainmenu()
def file():
    dirt = input(Fore.CYAN + '      Enter a file location: ')
...
\033[96m | $$\  $ | $$| $$_____/| $$      | $$      | $$  | $$| $$      | $$  | $$
 ...
 ''') % ip)
    ...
    '''))
    ans = input(Fore.WHITE + 'Enter a choice  ~# ')
    if ans == '0':
        readme()
    if ans == '1':
        brute_force()
